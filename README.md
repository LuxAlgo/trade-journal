...
| **Charts**               | A live Vela chart for any symbol from your market-data source, built for a stylus: the pen tip draws, the eraser end erases, fingers pan. Pine Script indicators (15 built-ins plus your own, written in the editor) run on PineTS. Your trades and missed trades appear on the chart (click to open), along with support/resistance zones that track touches and breaks, market session opens and an opt-in economic calendar. Analyses save automatically per symbol, with drawing layers in folders and alerts when price crosses your lines or zones or an indicator signals; embed them in the daily journal as images that reopen the chart, and AI reviews read them too. See [docs/charts.md](docs/charts.md). |
...
Chart indicators bundle [PineTS](https://github.com/LuxAlgo/PineTS) and `@luxalgo/vela-pinets`, which are AGPL-3.0; a deployment that includes them must meet AGPL-3.0 terms (see [docs/charts.md](docs/charts.md#license)). Code is licensed under [MIT](LICENSE) © [LuxAlgo Global, LLC](https://luxalgo.com). The project name and LuxAlgo marks are covered by the [trademark policy](TRADEMARKS.md). Security reports: see [SECURITY.md](SECURITY.md).
